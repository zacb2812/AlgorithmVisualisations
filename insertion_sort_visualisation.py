...
def insertion_sort(vis_info, ascending):
    lst = vis_info.lst

    for i in range(1, len(lst)):
        j = i
        while j > 0 and ((lst[j-1] > lst[j] and ascending) or (lst[j-1] < lst[j] and not ascending)):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r:
                    ...
                    sorting = True
                    algorithm_generator = insertion_sort(vis_info, ascending)
